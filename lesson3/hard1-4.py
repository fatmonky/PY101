...
def is_dot_separated_ip_address(input_string):
    dot_separated_words = input_string.split(".")
    if len(dot_separated_words) != 4: #extra condition
        return False
    while len(dot_separated_words) > 0:
        word = dot_separated_words.pop()
        if not is_an_ip_number(word):
            return False #change from break
    return True
...
# tests
print(is_dot_separated_ip_address("4.5.5")) #expect: False
print(is_dot_separated_ip_address("1.2.3.4.5")) #expect: False
print(is_dot_separated_ip_address("1.2.3.a")) #expect: False
print(is_dot_separated_ip_address("1.2.3.4")) #expect: True
